 ...
 * based on a region of a given size by applying a median threshold 
 * to the region and counting pixels with an intensity above the 
 * threshold as foreground and other pixels as background.
 * 
 * @author	Volker Baecker
 */
abstract public class MedianThresholdSignalToNoiseEstimator {
	...
	abstract protected double calculateLocalSNR(ImagePlus image, int threshold);
	...
	public MedianThresholdSignalToNoiseEstimator(ImagePlus image, int radiusX, int radiusY) {
		this.image = image;
	...
	static public MedianThresholdSignalToNoiseEstimator newFor(ImagePlus image, int radiusX, int radiusY) {
		MedianThresholdSignalToNoiseEstimator sne = null;
		
		if (image.getBitDepth()==8) {
			sne = new MedianThresholdSignalToNoiseEstimatorByte(image, radiusX, radiusY);
		}
		if (image.getBitDepth()==16) {
			sne = new MedianThresholdSignalToNoiseEstimatorShort(image, radiusX, radiusY);
		}
	...
	public void run() {
		SignalToNoiseRatioCalculator calc = SignalToNoiseRatioCalculator.newFor(image);
		int index=0;
		for (int y=0; y<height; y++) {
			for (int x=0; x<width; x++) {
				ImagePlus currentArea = this.getImageAround(x,y);
				BasicStatistics stats = BasicStatistics.newFor(currentArea);
				int threshold =(int) Math.round(stats.getMedian() + stats.getMedianStdDev());
				localSNR[index] = calc.calculateSNRForRegion(x, y, neighborhoodRadiusX, threshold);
				index++;
			}
		}
		BasicStatistics stats = new BasicStatisticsDouble(localSNR);
		medianSNR = stats.getMedian();
	}

	public ImagePlus getImageAround(int x, int y) {
	...
	public ImagePlus getLocalSNRImage() {
		...
		for (int i=0; i<localSNR.length; i++) floatLocalSNR[i] = (float)localSNR[i];
