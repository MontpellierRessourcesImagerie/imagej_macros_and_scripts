...
public class FilamentTracer {
	...
	protected float minTracingLength = 20;
	protected int stepSize = 2;
	protected SeedPointsFinder finder;
	
	public FilamentTracer(ImagePlus image, int maxFilamentWidth) {
		this.image = image;
	...
	public Vector<Tracing> traceBranches(Tracing mainTracing) {
		...
		while(it.hasNext()) {
			...
			aRoi.setImage(image);
			if (aRoi.getLength()>=minTracingLength) traces.add(aTrace);
		}
	...
	public Tracing traceFrom(int x, int y, int startDirection, Tracing trace) {
		...
		while (!isFinished(xn,yn, index)) {
			...
			xn=(int)Math.round((centerX+(stepSize*(xInc[filamentDirection]/length))));
			yn=(int)Math.round((centerY+(stepSize*(yInc[filamentDirection])/length)));
	...
	public void calculateMaxTemplateResponsesFor(int x, int y) {
		...
		for (int distance=0; distance<maxFilamentWidth/2; distance++) {
