...
var _MIN_NUCLEUS_SIZE =0.00004;
var _MIN_CELL_SIZE = 0.00005;
var _USE_WATERSHED_ON_SIGNAL = false;
var _MIN_SIGNAL_SIZE = 0.000085;
...
_SIGNAL_THRESHOLDING_METHOD = "MaxEntropy";
...
//	setBatchMode(true);
	...
	if (bestRoundness<0.000000000000000005) return result;
	result[0] = 1;
	result[1] = bestX;
	result[2] = bestY;
	result[3] = area;
...
function findCellsWith2Nuclei() {
	roiManager("reset")
	run("Z Project...", "projection=[Max Intensity]");
	run("Duplicate...", "duplicate channels=2-2");
	setAutoThreshold("MaxEntropy dark no-reset stack");
	run("Analyze Particles...", "size="+_MIN_NUCLEUS_SIZE+"-Infinity show=Masks exclude add");
	close();
	close();
	count = roiManager("count");
	indices = newArray(0);
	for (i = 0; i < count; i++) {
		...
		run("Fill Holes");
		run("Watershed");
		run("Analyze Particles...", "size="+_MIN_NUCLEUS_SIZE+"-Infinity clear display exclude");
		if (nResults!=2) {
