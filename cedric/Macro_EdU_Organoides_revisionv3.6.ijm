// =====================================================
// EdU/DAPI per-Z Counter — v3.5 + QA Overlay (Fiji macro)
...
// === Overlay QC coloré par Z ===
// DAPI_z (gris, contrasté) + contours ROIs colorés : Rouge=EdU+, Vert=EdU−
// Cartouche: Z / N / EdU+ (%)
// === Overlay QC coloré par Z ===
// DAPI_z (gris, contrasté) + contours ROIs colorés : Rouge=EdU+, Vert=EdU−
// Cartouche: Z / N / EdU+ (%)
function saveOverlayQA_Color(name, z, idDAPI, idEDU, outputDir) {
    // Image support = DAPI_z contrastée
    selectImage(idDAPI); Stack.setSlice(z);
    run("Duplicate...", "title=QA_overlay_z"+z);
    idOverlay = getImageID();
    run("Enhance Contrast", "saturated=0.35 normalize");

    // Image EdU 2D pour mesures + seuil par tranche
    ...
    // Dessin coloré des ROIs selon positivité EdU
    n = roiManager("count");
    setLineWidth(1);
    edupos_count = 0;
    ...
    for (r=0; r<n; r++) {
        ...
        // dessiner contour sur overlay
        selectImage(idOverlay);
        roiManager("select", r);
        if (isPos) {
            setForegroundColor(255, 0, 0); // rouge = EdU+
            edupos_count++;
        } else {
            setForegroundColor(0, 255, 0); // vert = EdU-
        }
        run("Draw");
    }
    ...
    if (n > 0) {
        pct = 100.0 * edupos_count / n;
    } else {
        pct = 0;
    }

    drawString("Z="+z+"   N="+n+"   EdU+="+edupos_count+" ("+d2s(pct,1)+"%)", 10, 20);
