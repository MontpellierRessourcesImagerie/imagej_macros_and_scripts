...
function prompt_settings() {
	...
	Dialog.addNumber("Min spots size (voxels)", 5);
	Dialog.addNumber("Max spots size (voxels)", 500);
	Dialog.addNumber("Min sphericity", 0.0);
	Dialog.addCheckbox("Normalize?", true);
	...
	do_normalize    = Dialog.getCheckbox();
}
