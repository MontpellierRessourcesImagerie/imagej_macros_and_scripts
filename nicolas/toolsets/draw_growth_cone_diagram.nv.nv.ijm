...
/********************   run("Images RGB To Stack") and make a montage **********/
...
M_Param_Input = "columns=" + M_Number_Cols + " rows=" + M_Number_Rows + " scale=0.3 label";
run("Gamma...");


// run("Make Montage...", "columns=3 rows=2 scale=0.50 label");
// run("Make Montage...","");
// Param_Input = "columns=" + M_Number_Col + " rows=" + M_Number_Rows + " scale=0.50 label";
// print(M_Param_Input);
// waitForUser;
...
function drawRois(ROI_File_Name,FileIndex) {
	...
	newImage(Image_Name, "16-bit black", 512, 512, 1);
