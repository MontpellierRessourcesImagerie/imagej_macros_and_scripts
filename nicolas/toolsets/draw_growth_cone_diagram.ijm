...
// colors = newArray(0xFF0000,0x00FFFF,0x0000FF,0x0000A0,0xAdD8E6,0x800080,0xFFFF00,0x00FF00,0xFF00FF,0xC0C0C0,0x808080,0x000000,0xFFA500,0xA52A2A,0x800000,0x008000,0x008000,0x808000);
...
for (i = 0; i < count; i++) {
	...
	color=0; 
	color = Math.sqrt( Math.pow(alpha,2) + Math.pow(beta,2) + Math.pow(gamma,2) );
	// color = (i+1)*Color_Pas_Ech;
