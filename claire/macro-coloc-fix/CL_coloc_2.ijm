...
for (j=0; j<list.length; j++) { // loop for all images of the folder
	if (endsWith(list[j], ".czi")) {	//works only with 2D images in czi (Zeiss) format
	...
	run("Convert to Mask");
	//run("Median...", "radius=3");
	//run("Auto Local Threshold", "method=Mean radius=100 parameter_1=0 parameter_2=0 white"); // detection of the nuclei with the DAPI signal
	...
	for (i=0; i<n; i++) { // loop for all the nuclei of the current image, meaning each ROI of the ROI manager
		...
		if (BnpB<50) {
			setResult("BnpB", (m+i), "0");// if the maximum intensity of the HA-BnpB signal is less than 50, the nucleus is in a cell not transfected, and the BnpB column will contain 0
		}
		else {
			setResult("BnpB", (m+i), "1"); // if the maximum intensity of the HA-BnpB signal is greater than 50, the nucleus is in a transfected cell, and the BnpB column will contain 1
		}
...
m = m + n; // m, number of the Results table	
Overlay.show();
